...
export type SocialNotificationPriority = "high" | "medium" | "low";

export type SocialNotificationTargetType =
  | "post"
  | "comment"
  | "profile"
  | "story"
...
export type CreateSocialNotificationInput = {
  recipientUserId: string;
  actorUserId: string;
  type: SocialNotificationType;
  priority: SocialNotificationPriority;
  targetType: SocialNotificationTargetType;
  referenceId: string;
  secondaryReferenceId?: string | null;
  friendshipId?: string | null;

  /// Optionale explizite Ziel-IDs. Bestehende Trigger müssen sie nicht
  /// zwingend setzen, weil referenceId/secondaryReferenceId weiterhin die
  /// kanonische Quelle bleiben.
  postId?: string | null;
  ...
  title: string;
  bodyBuilder: (actorLabel: string, groupCount: number) => string;
  isActionable?: boolean;
  friendRequestStatus?: "pending" | "accepted" | "removed" | "blocked" | null;
  pushEnabledByDefault?: boolean;
};
...
type UserNotificationSettings = {
  ...
  quietModeAllowSecurityAlerts: boolean;
};

export async function createOrGroupSocialNotification(
  input: CreateSocialNotificationInput,
): Promise<void> {
  ...
  if (!recipientUserId || !actorUserId) {
    logger.warn("SOCIAL_NOTIFICATION_SKIPPED_INVALID_USER", {
      recipientUserId,
      actorUserId,
      type: input.type,
    });
  ...
  if (!groupKey || !deduplicationKey || !referenceId) {
    logger.warn("SOCIAL_NOTIFICATION_SKIPPED_INVALID_TARGET", {
      recipientUserId,
      actorUserId,
      type: input.type,
      groupKey,
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const existingDeduplicationKeys = readStringArray(
      existingData.deduplicationKeys,
    );
    ...
    const actorUserIds = new Set<string>(
      readStringArray(existingData.actorUserIds),
    );
    ...
    finalGroupCount = Math.max(
      readPositiveNumber(existingData.groupCount) + 1,
      nextActorUserIds.length,
      1,
    );
...
export async function deleteSocialNotificationByGroupKey(
  recipientUserId: string,
  groupKey: string,
): Promise<void> {
  ...
  if (!cleanedRecipientId || !cleanedGroupKey) return;
...
async function loadActorSnapshot(
  ...
  const displayName =
    cleanString(data.displayName) || cleanString(data.name);
...
async function loadNotificationSettings(
  ...
  return {
    pushNotificationsEnabled:
      readBool(data.pushNotificationsEnabled, true),
    inAppNotificationsEnabled:
      readBool(data.inAppNotificationsEnabled, true),
    likeNotificationsEnabled:
      readBool(data.likeNotificationsEnabled, true),
    commentNotificationsEnabled:
      readBool(data.commentNotificationsEnabled, true),
    replyNotificationsEnabled:
      readBool(data.replyNotificationsEnabled, true),
    friendRequestNotificationsEnabled:
      readBool(data.friendRequestNotificationsEnabled, true),
    mentionNotificationsEnabled:
      readBool(data.mentionNotificationsEnabled, true),
    quietModeEnabled:
      readBool(data.quietModeEnabled, false),
    quietModeAllowSecurityAlerts:
      readBool(data.quietModeAllowSecurityAlerts, true),
  };
}
...
function shouldSendPushForType(
  ...
  if (type === "securityAlert") return true;
...
async function sendPushToUser(input: SendPushInput): Promise<void> {
  ...
  const invalidTokenDocuments: FirebaseFirestore.QueryDocumentSnapshot[] = [];
  ...
  response.responses.forEach((result, index) => {
    ...
    if (
      ...
      logger.error("SOCIAL_PUSH_SEND_FAILED", {
  ...
  if (invalidTokenDocuments.length === 0) return;
...
function buildWebPushLink(input: SendPushInput): string {
  ...
  if (input.postId) parameters.set("postId", input.postId);
  if (input.commentId) parameters.set("commentId", input.commentId);
  if (input.profileId) parameters.set("profileId", input.profileId);
  if (input.storyId) parameters.set("storyId", input.storyId);
